namespace Thiesen.Application.Resources
{
    public static class ValidationMessages
    {
        public const string NotFoundUsuario = "Usuário Inexistente!";
        public const string NotFoundPessoaFisica = "Pessoa Física Inexistente!";

        public const string RequiredNome = "Informe o Nome!";
        public const string RequiredCPF = "Informe um CPF!";
        public const string RequiredNomeMae = "Informe o Nome da Mãe!";
        public const string RequiredDataNascimento = "Informe a Data de Nascimento!";
        public const string RequiredLogin = "Informe o login!";
        public const string RequiredPassword = "Informe a senha!";

        public const string InvalidCPF = "CPF inválido!";
        public const string InvalidDataNascimento = "Data de Nascimento inválida!";
        public const string InvalidLogin = "Login inválido!";
        public const string InvalidEmail = "Email inválido!";
    }
}
